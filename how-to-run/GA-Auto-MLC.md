# GA-Auto-MLC
Automated Multi-Label Classification on the MEKA software using Genetic Algorithm

## **How to Run**

Run the following command line in the directory you downloaded from github:

> /home/<your_user_dir>/<...>/jdk1.8.0_171/jre/bin/java -Xmx8g -cp automeka.jar meka.classifiers.multilabel.meta.GAAutoMLC -t datasets/01-Flags-Stratified5FoldsCV-Meka/Flags-train1.arff -T datasets/01-Flags-Stratified5FoldsCV-Meka/Flags-test1.arff
...
- -O <value>: to set the search space mode, i.e., which search space the GGP will use to guide its search. The options are: 
  ...
  - 1: Medium
  - 2: Large (default)
- -A <XML_File>: to set the XML file describing the search space. The defined options are available in the same directory of this project: 
  - "algorithms-minimal.xml" contains all the options for the search space minimal.
  - "algorithms-medium.xml" contains all the options for the search space medium.
  - "algorithms-large.xml" contains all the options for the search space large.  
