...
public class ApplicabilityTester
{
    ...
    public static class ApplicableClassifiers
    {
        ...
        public List<ClassParams> baseMLC;
        public List<ClassParams> metaMLC;
    }
    ...
    public static ApplicableClassifiers getApplicableClassifiers(Instances instances, String baseDir, List<String> allowed)
    {
        ...
        //And do all the multi-label classification methods
        options.clear();
        options.add("-W");
        options.add(app.base.get(0).getTargetClass());

        dir = baseDir + File.separator + "mlc_base" + File.separator; 
        app.baseMLC = getApplicable(instances, dir, Testable.MLC_CLASSIFIER, allowed, options);
        
        //And do all the meta multi-label classification methods
        options.clear();
        options.add("-W");
        options.add(app.base.get(0).getTargetClass());
        options.add("-W");
        options.add(app.baseMLC.get(0).getTargetClass());        

        dir = baseDir + File.separator + "mlc_meta" + File.separator; 
        app.metaMLC = getApplicable(instances, dir, Testable.MLC_CLASSIFIER, allowed, options);        
//        
//        for(ClassParams cp : app.metaMLC){
//            System.out.println(cp.getTargetClass());
//        }
    ...
    private static boolean isApplicable(String method, Instances instances, List<String> options, Testable type)
    {
        switch(type){
            ...
            case MLC_CLASSIFIER:
                return isApplicableMLClassifier(method, instances, options);
            default:
                return false;
        }
    }
    ...
    /**
     * Check if the multi-label classifier is applicable to that dataset.
     * @param method the MLC method
     * @param instances the MLC instances
     * @param options the method's options
     * @return the applicability of the method to the given instances
     */
    private static boolean isApplicableMLClassifier(String method, Instances instances, List<String> options){
        boolean isApplicable = false;

        try{
            // Create an instance of the multi-label classifier:
            MultiLabelClassifier mlc_classifier = (MultiLabelClassifier) AbstractMultiLabelClassifier.forName(method, new String[]{""});
            if(options != null){
                try{
                   mlc_classifier.setOptions(options.toArray(new String[options.size()])); 
                }catch(Exception e){
            ...
            mlc_classifier.getCapabilities().testWithFail(instances);
            isApplicable = true;
            
        }catch(ClassNotFoundException e){
            log.debug("No class {} found", e, method);
