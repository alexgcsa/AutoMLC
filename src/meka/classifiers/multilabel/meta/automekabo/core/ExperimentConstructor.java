...
//        classParams.add(mAttribSearchClassParams);
    ...
    public ParameterConditionalGroup generateAlgorithmParameterConditionalGroupForDAG()
    {
        ...
        List<String> mlc_metaClassifiers = new ArrayList<String>();
        ...
        if(mIncludeMeta){
            for(ClassParams clsParams: mMetaClassParams) {
                ...
                //To indicate that the meta-algorithms are not going to be used as well.
                metaClassifiers.add("NO_META_SLC"+meta_slc);
                meta_slc++;
            }
        }
...
//            classifiers.add(className);
        ...
        //Go build up the names of meta multi-label classification methods
        int meta_mlc = 0;       
        for(ClassParams clsParams: mMLCMetaClassParams) {
            String className = clsParams.getTargetClass();
            classifiers.add(className);
            mlc_metaClassifiers.add(className);
            mlc_metaClassifiers.add("NO_META_MLC" + meta_mlc);
            classifiers.add("NO_META_MLC" + meta_mlc);             
            meta_mlc++;
        }         
//        
...
//        //Build the entire list of all classifiers as a parameter, and insert it
        Parameter targetclass = new Parameter("targetclass", classifiers, "meka.classifiers.multilabel.meta.RandomSubspaceML");
        paramGroup.add(targetclass);    
        
        if (mIncludeMLCMeta) {
            for (ClassParams clsParams : mMLCMetaClassParams) {
                addClassifierToParameterConditionalGroupForDAG(paramGroup, clsParams, "0_", targetclass);
            }
        }  
   

        if (!metaClassifiers.isEmpty()) {            
            Parameter _1_W = new Parameter("1_W", mlc_baseClassifiers);
            paramGroup.add(_1_W);
            paramGroup.add(new Conditional(_1_W, targetclass, mlc_metaClassifiers));
            Parameter _1_W_0_DASHDASH = new Parameter("1_W_0_DASHDASH", "REMOVED");            
            
            Parameter _2_W = new Parameter("2_W", metaClassifiers);
            paramGroup.add(_2_W);
            paramGroup.add(new Conditional(_2_W, targetclass, mlc_metaClassifiers));
            Parameter _2_W_0_DASHDASH = new Parameter("2_W_0_DASHDASH", "REMOVED");

            Parameter _3_W = new Parameter("3_W", baseClassifiers);
            paramGroup.add(_3_W);
            paramGroup.add(new Conditional(_3_W, targetclass, mlc_metaClassifiers));
            Parameter _3_W_0_DASHDASH = new Parameter("3_W_0_DASHDASH", "REMOVED");
            
            paramGroup.add(_1_W_0_DASHDASH);
            paramGroup.add(new Conditional(_1_W_0_DASHDASH, targetclass, mlc_metaClassifiers));
            
            paramGroup.add(_2_W_0_DASHDASH);
            paramGroup.add(new Conditional(_2_W_0_DASHDASH, targetclass, mlc_metaClassifiers));

            paramGroup.add(_3_W_0_DASHDASH);
            paramGroup.add(new Conditional(_3_W_0_DASHDASH, targetclass, mlc_metaClassifiers));
            
            for (ClassParams clsParams : mMLCBaseClassParams) {
                addClassifierToParameterConditionalGroupForDAG(paramGroup, clsParams, "1_W_1_", _1_W);
            }
            for (ClassParams clsParams : mMetaClassParams) {
                addClassifierToParameterConditionalGroupForDAG(paramGroup, clsParams, "2_W_2_", _2_W);
            }
            for (ClassParams clsParams : mBaseClassParams) {
                addClassifierToParameterConditionalGroupForDAG(paramGroup, clsParams, "3_W_3_", _3_W);
            }
        }
