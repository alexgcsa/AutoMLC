...
//        this.seed = seed;
    ...
    public TranslateIndividual(String indFromGrammar, long seed, int specie) {
        this.indFromGrammar = indFromGrammar;
        this.specie = specie;
//        this.seed = seed;
        /** Mapping methods for SLC. */
    ...
    @Override
    public String translate2Command(String trainingSet, String validationSet, int timeoutLimit, boolean completeCommand, String javaDir){
        ...
        if(completeCommand){
            command  = "timeout " + timeoutLimit+"s " +javaDir+" -Xmx2g -cp weka.jar:meka.jar ";
        }
...
//        transl += sep + "-S" + sep + this.seed;
    ...
    public TranslationProcess translate_RandomTree(String[] classifierName, String transl_partial, int pos_partial, String sep) {
        ...
        transl += sep + "-M" + sep + classifierName[pos];
        pos++;
        if(pos >= classifierName.length){
            System.err.println("##############");
            System.err.println("ERRO NO PIPELINE: "+Arrays.toString(classifierName) +" -- na specie "+specie);
        }
        transl += sep + "-N" + sep + classifierName[pos];
        pos++;      
        if(pos >= classifierName.length){
